...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!body.invoiceId || !body.invoiceId.trim()) {
      return NextResponse.json(
        { success: false, error: "Invoice ID is required for returns." },
        { status: 400 }
      );
    }
    ...
    if (product.serialTracking) {
      if (!serialNumbers || !Array.isArray(serialNumbers) || serialNumbers.length !== quantity) {
        return NextResponse.json(
          { success: false, error: `Serial tracking is enabled for this product. Please provide exactly ${quantity} serial numbers to return.` },
          { status: 400 }
        );
      }
      ...
      for (const serialItem of serialNumbers) {
        try {
          ...
          // Check if serial was sold and matches the invoice
          if (serial.status !== "Sold") {
            results.failed.push({ serialNumber: serialNumberValue, error: `Serial number is not in Sold status (current status: ${serial.status})` });
            continue;
          }

          if (serial.invoiceId?.toString() !== body.invoiceId) {
            results.failed.push({ serialNumber: serialNumberValue, error: "Serial number was not sold under the provided invoice" });
            continue;
          }

          // Update serial status to Returned
          serial.status = "Returned";
          serial.returnDate = new Date();
      ...
      return NextResponse.json({ 
        ...
        summary: {
          ...
          message: `Return completed for product "${product.name}". ${results.successful.length} serial numbers marked as returned.`,
        }
      });
    } else {
      ...
      return NextResponse.json({ 
        ...
        data: {
          ...
          message: `Return completed for product "${product.name}". Quantity: ${quantity}. Serial tracking is disabled - quantity-based inventory.`,
        }
      });
    }
  } catch (error: any) {
